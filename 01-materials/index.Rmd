...
date: "0900&ndash;1500 CET Tuesday 9th June, 2026"
output:
...
# GAMs are (just) fancy GLMs

To make effective use of GAMs we need to grasp the basics of GLMs

A **G**eneralized **L**inear **M**odel is a statistical model that allows the response to be conditionally distributed according to a wider set of distributions than the Gaussian of linear regression
...
Flip a coin 10 times with *p* = 0.7, the probability of 7 heads is $\sim \mathcal{B}(n = 10, p = 0.7)$, ~ 0.27
...
```{r binomial-pdf, echo = FALSE, fig.height=4}
...
```{r poisson-pdf, echo = FALSE, fig.height=4}
s <- seq(0, 20, by = 1)
poisson.pmf <- data.frame(
  x = rep(s, 3),
...
plt.poisson <- ggplot(poisson.pmf, aes(x = x, y = pmf, fill = params)) +
  geom_col() + labs(y = "Probability Mass", x = "Count", fill = NULL) +
  facet_wrap(~ params) + scale_fill_okabe_ito() +
  theme(legend.position = "none")
plt.poisson
```
...
General ideas we will encounter with GAMs extend to distributions beyond this family
...
This doesn't work out the same for GLMs &mdash; we don't (usually) have a residual term $\varepsilon$ in the linear predictor
...
.center[
![REML and GCV smoothness selection](./resources/remlgcv.png)
]
...
<https://gavinsimpson.github.io/au-viborg-gam-course/day-1/milk-yield.html>

You can also download the [`milk-yield.qmd`](https://github.com/gavinsimpson/au-viborg-gam-course/raw/refs/heads/main/day-1/milk-yield.qmd) and open it in RStudio
...
Code for this is in `01-materials/test.R`
